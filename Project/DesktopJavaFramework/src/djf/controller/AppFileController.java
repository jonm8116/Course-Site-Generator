...
import static djf.settings.AppPropertyType.COMBOBOX_PROCEED_MESSAGE;
import static djf.settings.AppPropertyType.COMBOBOX_PROCEED_TITLE;
...
public class AppFileController {
    ...
    /**This method pulls up the about page that provides information
     * on this application whenever the user clicks on the about icon
     */
    ...
    //METHOD TO HANDLE EXPORT REQUEST
...
//        
//        filePath.replace("TAManager_Solution", "TAManagerTester/public_html");
        //File sourceDir = new File(filePath);
        String filePath = System.getProperty("user.dir");
        filePath +="/export_dir/public_html";
        File sourceDir = new File(filePath);
        boolean result = sourceDir.exists();
        File exportedJson = new File(filePath+ File.separator +"js" + File.separator +"OfficeHoursGridData.json");
        
        try{
            FileUtils.copyDirectory(sourceDir, exportedDirectory);
            exportedJson.getParentFile().mkdirs(); 
            boolean ifSuccess = exportedJson.createNewFile();
            FileReader readFile = new FileReader(currentWorkFile);
            FileWriter writeFile = new FileWriter(exportedJson);
            
            int readLine = readFile.read();
            while(readLine!=-1) {
                writeFile.write(readLine);
                readLine = readFile.read();
            }
            //app.getFileComponent().exportData(app.getDataComponent(), filePath);
            //FileUtils.copyFile(currentWorkFile, exportedJson);
        } catch(IOException ex){
            System.out.print("io exception occur");
        } catch(NullPointerException ex){
           // System.out.print("null");
        }
        //copyDir(sourceDir, exportedDirectory);
//        File courseDetailsJson = new File(filePath+ File.separator +"js" + File.separator +"CourseDetails.json");
        try{
            String path = exportedDirectory.getAbsolutePath();
            path = path + File.separator + "js" + File.separator;
            app.getFileComponent().exportData(app.getDataComponent(), path);
        } catch(IOException ex){
    ...
    public boolean promptProceedComboBox(){
        PropertiesManager props = PropertiesManager.getPropertiesManager();
	...
	// BEEN SAVED AT LEAST ONCE
        ...
        // PROMPT THE USER TO SAVE UNSAVED WORK
	AppYesNoCancelDialogSingleton yesNoDialog = AppYesNoCancelDialogSingleton.getSingleton();
        yesNoDialog.show(props.getProperty(COMBOBOX_PROCEED_TITLE), props.getProperty(COMBOBOX_PROCEED_MESSAGE));
        ...
        // CALLED THIS THAT THE USER IS NOT INTERESTED ANYMORE
                ...
                selection.equals(AppYesNoCancelDialogSingleton.NO)) {
            //return false;
            return false;
        }
